...
class AVLTree:
    ...
    def insert(self, node, metro_stop):
        if node is None:
            nn=AVLNode(metro_stop)
            
            
            return nn
        compare_result = self.string_compare(metro_stop, node.get_stop_name())
        ...
        if compare_result == 0:
            ...
            new_n=self.insert(node.get_left(), metro_stop)
            

            node.set_left(new_n)
            new_n.set_parent(node)
        else:
            n_node=self.insert(node.get_right(), metro_stop)
            node.set_right(n_node)
            n_node.set_parent(node)

        # Balance the tree after insertion
        return self.balance(node)  # Implement this method

    def populate_tree(self, metro_line):

        node=metro_line.get_node()
        while(node!=None):
            self.set_root(self.insert(self.get_root(), node.get_stop_name()))
